...
rARMA <- function(n, d, Phi, Theta, Sigma, burn = 100, freq = NULL) {
  ...
  if (!isTRUE(all(eigen(Sigma, symmetric = TRUE, only.values = TRUE)$values >= 0))) {
    stop("Sigma is not a PSD matrix")
  }

  ## Generate time series
  Se <- Sqrt(Sigma)
  Z <- replicate(n + burn, Se %*% stats::rnorm(d), simplify = TRUE)
...
rExamples1D <- function(n, d = 3, example = c("bumps", "two-cats", "heaviSine", "gaussian", "mix-gaussian", "arma"), user.f = NULL,
  ...
  if(!is.null(user.f)) {
    ## Use 'user.f' if supplied
    if(!(is.array(user.f) && dim(user.f) == c(d, d, n))) {
      stop("'user.f' should be a (d,d,n)-dimensional real or complex array")
    }
    f <- user.f
  ...
  for(s in 1:replicates) {
    ...
    if(return.ts | noise == "periodogram") {
      ...
      ts[, , s] <- sqrt(2 * pi) / sqrt(2 * n) * mvfft(t(sapply(1:(2 * n), function(i) f2[, , i] %*% chi[, i])), inverse = TRUE)
    ...
    if(noise %in% c("riem-gaussian", "log-gaussian")) {
      ...
      W0 <- replicate(n, H.coeff(rnorm(d^2, sd = sqrt(noise.level)), inverse = TRUE))
      if(noise == "riem-gaussian") {
        W <- Ptransf2D_C(W0, TRUE, FALSE, "logEuclidean")
        P[, , , s] <- sapply(1:n, function(i) (t(Conj(f1[, , i])) %*% W[, , i]) %*% f1[, , i], simplify = "array")
      } else if(noise == "log-gaussian") {
        P[, , , s] <- Ptransf2D_C(f1 + W0, TRUE, FALSE, "logEuclidean")
      }
      ...
      if(noise.level != 1 & df.wishart >= d) {
        W <- Ptransf2D_C(sqrt(noise.level) * Ptransf2D_C(W, FALSE, FALSE, "logEuclidean"), TRUE, FALSE, "logEuclidean")
      }
      if(noise == "wishart") {
        P[, , , s] <- sapply(1:n, function(i) (t(Conj(f1[, , i])) %*% W[, , i]) %*% f1[, , i], simplify = "array")
      } else {
        P[, , , s] <- Ptransf2D_C(f1 + Ptransf2D_C(W, FALSE, FALSE, "logEuclidean"), TRUE, FALSE, "logEuclidean")
      }
    }
  }
...
rExamples2D <- function(n, d = 2, example = c("smiley", "tvar", "facets", "peak"), replicates = 1,
  ...
  if(example == "smiley"){
    ...
    eye1 <- c(1/3, 2/3) * n
    eye2 <- c(2/3, 2/3) * n
    p_i <- replicate(4, Expm(diag(2, d), H.coeff(rnorm(d^2, sd = 2), inverse = TRUE)))
    for(i1 in 1:n[1]){
      for(i2 in 1:n[2]){
    ...
    Sigma <- Expm(diag(d), H.coeff(rnorm(d^2), inverse = TRUE))
  ...
  f1 <- Ptransf2D(f, FALSE, ifelse(noise %in% c("riem-gaussian","wishart"), "rootEuclidean", "logEuclidean"))
  ...
  for(s in 1:replicates) {
    ...
    if(noise %in% c("riem-gaussian", "log-gaussian")) {
      ## Generate iid zero mean (intrinsic) gaussian noise
      W0 <- replicate(n[2], replicate(n[1], H.coeff(rnorm(d^2, sd = sqrt(noise.level)), inverse = TRUE)))
      if(noise == "riem-gaussian") {
        W <- Ptransf2D(W0, TRUE, "logEuclidean")
        P[, , , , s] <- array(c(mapply(function(i, j) ast(f1[, , i, j], W[, , i, j]), grid.n$Var1, grid.n$Var2)), dim = dim(W))
      } else if(noise == "log-gaussian") {
        P[, , , , s] <- Ptransf2D(f1 + W0, TRUE, "logEuclidean")
      }
    } else if(noise %in% c("wishart", "log-wishart")) {
      ## Generate iid zero mean (intrinsic) wishart noise
