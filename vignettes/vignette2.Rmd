...
- id: COvS17
  ...
  container-title: Available on ArXiv
  issued: 
    year: 2017
---
...
**Remark:** The internal function `pdSpecEst:::E_basis(2)` constructs an orthonormal basis of the tangent space at the identity element `diag(2)`, and the internal function `pdSpecEst:::E_coeff_inv()` converts (real-valued) basis components to tangent space elements using the constructed orthonormal basis. See [@COvS17] for more details.
...
It is interesting to compare the most central HPD matrix above with the (approximate) empirical geometric mean of the observations obtained with `KarchMean()`. The empirical geometric mean is known to maximize the data depth for observations from a *centrally symmetric* distribution (as in this example). For more details, see [@COvS17].
