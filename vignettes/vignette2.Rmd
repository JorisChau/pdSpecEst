...
references:
- id: COvS17
  title: Statistical data depth and rank-based tests for spectral density matrices
  author:
  - family: Chau  
    given: J.
  - family: Ombao
...
**Remark:** The internal function `pdSpecEst:::E_basis(2)` constructs an orthonormal basis of the tangent space at the identity element `diag(2)`, and the internal function `pdSpecEst:::E_coeff_inv()` converts (real-valued) basis components to tangent space elements using the constructed orthonormal basis.
...
Let us compare the most central HPD matrix with the (approximate) empirical geometric mean obtained with `KarchMean()`, which we know maximizes the data depth for *centrally symmetric* distributions. For additional details, see [@COvS17].
...
**Remark:** The internal function `pdSpecEst:::T_basis()` constructs an orthonormal basis of the tangent space attached to a specific point (HPD matrix) in the Riemannian manifold of HPD matrices as detailed in [@COvS17].

The manifold signed-rank test also provides a valid procedure to test for equivalence of spectral matrices of two (independent) multivariate stationary time series based on the HPD periodogram matrices obtained via `pdPgram()` as illustrated below. In contrast to other available tests in the literature, this asymptotic test does not require consistent spectral estimators or resampling/bootstrapping of test statistics, and therefore remains computationally efficient for higher-dimensional spectral matrices or a large number of sampled Fourier frequencies.
...
To conclude, let us consider several examples of the manifold Bartels-von Neumman test (`"bartels"`). Below, we generate an independent sample with a gradual trend in the scale of the distribution, such that the null hypothesis of randomness breaks down.
